...
function showCourse(domobj, which) {
	let coursebubblewidth;
	if (typeof coursebubblewidth == "undefined") {
		if(typeof bubblewidth == "undefined")
			{
				coursebubblewidth = 400;
			}
		else
			{
				coursebubblewidth = bubblewidth;
			}
	}
	function showCourseReady(req) {
		...
		if($(req).find("course").length) {
			...
			html = "<p>Course information cannot be found. This course may " +
	...
	$(domobj).blur();
	if(typeof ribbiturl == "undefined" || window.location.host.indexOf(".leepfrog.com") >= 0 || window.location.host.indexOf(".editcl.com") >= 0) {
...
function showCourseEco(domobj, which) {
	...
	if(typeof ribbiturl == "undefined" || window.location.host.indexOf(".leepfrog.com") >= 0)
		ribbiturl = "https://catalog.western.edu/ribbit/index.cgi";
	$.ajax({
